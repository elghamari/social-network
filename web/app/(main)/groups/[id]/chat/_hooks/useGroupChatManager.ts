...
export function useGroupChatManager() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      setMessages(mapped);
      if (mapped.length > 0) await markGroupAsRead(Number(group.id), mapped[mapped.length - 1].id);
    ...
    const handleWsMessage = (event: MessageEvent) => {
      try {
        ...
        if (parsed.type === "new_message") {
          ...
          if (incomingGroupId !== "undefined" && incomingGroupId === currentGroupId) {
            ...
            else if (String(newMsg.senderId) !== String(user?.id)) setHasUnreadBelow(true);
          }
        }
  ...
  return {
    messages, isLoadingMore, hasUnreadBelow, scrollContainerRef,
    handleTrackScroll, scrollToBottom, handleSendMessage, user,
    observerTarget: (node: any) => {}
  };
}