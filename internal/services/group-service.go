...
func (s *GroupService) ensureTabIsValid(tab string) error {
	validTabs := map[string]bool{"discover": true, "joined": true, "requests": true, "invitations": true}

	if !validTabs[tab] {
		return types.NewActionError("Not a valid tab Try: (discover || joined || requests || invitations)")
	}
...
func (s *GroupService) ensureEventResponseIsValid(er types.EventResponse) *types.ActionError {
...
func (s *GroupService) ListGroups(userId, tab, query, cursor string) ([]types.Group, error) {
	...
	if err := s.ensureTabIsValid(tab); err != nil {
		return nil, err
	}
...
func (s *GroupService) ApproveJoinRequest(groupId, approverId, userId string) error {
	...
	if !hasRequest {
		return types.NewActionError("No request found")
	}
...
func (s *GroupService) GetJoinRequestNotification(groupId, userId string) (types.Notification, error) {
	...
	content := fmt.Sprintf("requested to join %s", data.GroupTitle)
...
func (s *GroupService) RespondToEvent(groupId, userId string, er types.EventResponse) error {
	...
	if err := s.ensureEventResponseIsValid(er); err != nil {
		return err
	}
